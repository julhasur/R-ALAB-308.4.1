...
//Part 2
//Expading functionality
function findDynamicRow(str){
  let rows=[];
  let currentrow="";
  for(let i=0;i<str.length;i++){
    if(str[i]==="\n"){
 ...
 let dynamicrow=findDynamicRow("ID,Name,Occupation,Age\n42,Bruce,Knight,41\n57,Bob,Fry Cook,19\n63,Blaine,Quiz Master,58\n98,Bill,Doctor’s Assistant,26");
 console.log(dynamicrow);

 function twodArray(rows){
  let twodArray=[]
  for(let i=0;i<rows.length;i++){
    let rowarray=[];
    let currentrow="";
...
let csvarray= twodArray(dynamicrow);
console.log("formated csv:",csvarray);
console.log("number of columns:",csvarray[0].length);
